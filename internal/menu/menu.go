...
func PrintMenuOptions(preCheck utils.NetCheckResult, config *params.Config) {
	...
	case "zh":
		...
		fmt.Println("3. 极简版(系统信息+CPU+内存+磁盘+测速节点5个)")
		fmt.Println("4. 精简版(系统信息+CPU+内存+磁盘+跨国平台解锁+路由+TCP握手+测速节点5个)")
		fmt.Println("5. 精简网络版(系统信息+CPU+内存+磁盘+回程+路由+TCP握手+测速节点5个)")
		fmt.Println("6. 精简解锁版(系统信息+CPU+内存+磁盘IO+跨国平台解锁+测速节点5个)")
		fmt.Println("7. 网络单项(IP质量检测+上游及三网回程+广州三网回程详细路由+全国延迟+TCP握手+TGDC+网站延迟+测速节点11个)")
